...
def smart_search(query: str = Query(...), db: Session = Depends(get_db)):
    ...
    # 1. البحث الذكي في جدول الباحثين عن عمل (JobSeekers)
    # يبحث في: الاسم الأول، الأخير، المسمى الوظيفي، ومحتوى السي في
    seekers = db.query(JobSeekerDB).filter(
    ...
    # 2. البحث الذكي في جدول أصحاب العمل (Managers)
    # يبحث في: الاسم الأول، الأخير، واسم الشركة
    managers = db.query(ManagerDB).filter(
    ...
    # تحويل نتائج الباحثين عن عمل لتناسب كود Flutter
    for s in seekers:
        final_results.append({
            ...
            "user_type": "jobseeker", # هذا مهم جداً للـ OtherUserProfile
            "cv_file": s.cv_file or "" # للفتح المباشر إذا احتجنا
        })

    # تحويل نتائج أصحاب العمل لتناسب كود Flutter
    for m in managers:
        final_results.append({
            "id": m.id,
            ...
            "user_type": "manager", # يوجه التطبيق لواجهة المدير
            "cv_file": "" # المدير ليس لديه سي في
        })
