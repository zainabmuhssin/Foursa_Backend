...
def smart_search(
    ...
    if query:
        site_query = site_query.filter(or_(User.full_name.ilike(search_term), User.info.ilike(search_term)))
    if city:
    ...
    if role == "jobseeker":
        app_seekers = db.query(JobSeekerDB).filter(or_(JobSeekerDB.first_name.ilike(search_term), JobSeekerDB.job_title.ilike(search_term))).all()
        ...
        app_managers = db.query(ManagerDB).filter(or_(ManagerDB.first_name.ilike(search_term), ManagerDB.company_name.ilike(search_term))).all()
