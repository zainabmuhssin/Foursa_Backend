...
async def upload_profile_image(
    ...
    # 1. تحديد اسم فريد للملف لمنع التكرار
    file_extension = os.path.splitext(file.filename)[1]
    unique_filename = f"user_{email.split('@')[0]}{file_extension}"
    file_path = os.path.join(UPLOAD_DIR, unique_filename)

    # 2. ✅ السطر السحري: حفظ الصورة فعلياً في المجلد
    try:
    ...
    user.profile_image = unique_filename  # نخزن اسم الملف فقط
    db.commit()
