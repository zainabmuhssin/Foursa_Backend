...
from models import ContactMessage, JobSeekerDB, ManagerDB, Base
...
@router.post("/contact")
async def send_contact_message(data: dict, db: Session = Depends(get_db)):
    user_message = data.get("message")

    if not user_message:
        raise HTTPException(status_code=400, detail="Message cannot be empty")

    new_entry = ContactMessage(message=user_message)
    db.add(new_entry)
    db.commit()
